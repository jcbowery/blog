defmodule Blog.Accounts do
  ...
  alias Blog.Accounts.{User, UserToken, UserNotifier, Role}
  ...
  @doc """
  Adds a role to a user, ensuring no duplicate roles are assigned.
  """
  ...
  @doc """
  Removes a role from a user, ensuring no error on attempts from non present role in user
  """
  def remove_role_from_user(%{"user" => user_id, "role" => role}) do
    user = Repo.get!(User, user_id) |> Repo.preload(:roles)
    role = Repo.get_by!(Role, name: role)
  ...
  def delete_user(%{"user_id" => user_id}) do
    with %User{} = user <- Repo.get(User, user_id) |> Repo.preload(:roles) do
      user
      |> Ecto.Changeset.change()
      |> Repo.delete()
    else
